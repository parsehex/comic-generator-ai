# from https://github.com/lucasnewman/f5-tts-mlx/blob/main/examples/generate.py
# i've added a couple assertions to fix types
...
def generate(
    ...
    speed:
    float = 1.0,  # used when duration is None as part of the duration heuristic
	...
	if ref_audio_text is None:
		raise ValueError("Reference audio text must be provided")
	...
	if duration is None:
		...
		zh_pause_punc = r"。，、；：？！"
		ref_text_len = len(ref_audio_text.encode('utf-8')) + 3 * len(
		    re.findall(zh_pause_punc, ref_audio_text))
		gen_text_len = len(generation_text.encode('utf-8')) + 3 * len(
		    re.findall(zh_pause_punc, generation_text))
		duration_in_frames = ref_audio_len + int(
	...
	print(
	    f"Generated {generated_duration:.2f} seconds of audio in {elapsed_time}."
	)
