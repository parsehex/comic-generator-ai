...
"""
prompt_instructions = f'<prompt-instructions>\n{prompt_instructions}\n</prompt-instructions>'

prompt = f"""The following is text extracted from a webpage (<input-text>), as well as instructions for writing a prompt (<prompt-instructions) for the TTS model that the <input-text> will be fed into.

Assistant's task is to format and correct the <input-text>, using the <prompt-instructions> to better inform on how to do so. Assistant should respond with the corrected <input-text> within an <output> tag.
